...
export async function requireAuth(
  ...
  try {
    ...
    if (
      roleRequiresOfficeAssignment(roleName, isAdmin, isManager) &&
    ...
    let permissions = [
      ...new Set(
        authenticatedUser.role?.rolePermissions.flatMap((entry) =>
          permissionIdentifiers(entry.permission),
        ) ?? [],
      ),
    ];
...
export async function optionalAuth(
  ...
  try {
    ...
    req.permissions = [
      ...new Set(
        user.role?.rolePermissions.flatMap((entry) =>
          permissionIdentifiers(entry.permission),
        ) ?? [],
      ),
    ];
