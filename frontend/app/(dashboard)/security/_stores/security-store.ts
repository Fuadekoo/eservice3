...
type SecurityStore = {
  ...
  updatePermission: (id: string, payload: UpdatePermissionPayload) => Promise<Permission>;
...
export const useSecurityStore = create<SecurityStore>((set) => ({
  ...
  fetchRoles: async () => {
    ...
    try {
      const response = await axiosInstance.get<{ data: Role[] }>("/security/roles") as unknown as { data: Role[] };
  ...
  createRole: async (payload) => {
    try {
      const response = await axiosInstance.post<{ data: Role }>("/security/roles", payload) as unknown as { data: Role };
  ...
  updateRole: async (id, payload) => {
    try {
      const response = await axiosInstance.put<{ data: Role }>(`/security/roles/${id}`, payload) as unknown as { data: Role };
  ...
  fetchPermissions: async () => {
    set({ isLoading: true, error: null });
    try {
      const response = await axiosInstance.get<{ data: Permission[] }>("/security/permissions") as unknown as { data: Permission[] };
  ...
  createPermission: async (payload) => {
    try {
      const response = await axiosInstance.post<{ data: Permission }>("/security/permissions", payload) as unknown as { data: Permission };
  ...
  updatePermission: async (id, payload) => {
    try {
      const response = await axiosInstance.put<{ data: Permission }>(`/security/permissions/${id}`, payload) as unknown as { data: Permission };
      const updatedPermission = response.data;
      set((state) => ({
        permissions: state.permissions.map((permission) => (permission.id === id ? updatedPermission : permission)),
      }));
