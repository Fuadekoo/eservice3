...
export default function PermissionsPage() {
  ...
  const filteredPermissions = securityStore.permissions.filter(
    (p) =>
  ...
  // Group permissions by prefix (e.g. "stock", "staff", "finance")
  const groupedPermissions = filteredPermissions.reduce<Record<string, Permission[]>>(
    (acc, p) => {
      const prefix = p.code?.split(".")[0] || "general";
        ...
        <section>
          <Card>
            <CardHeader className="pb-4">
              ...
              <CardDescription>
                {filteredPermissions.length} permission{filteredPermissions.length !== 1 ? "s" : ""} defined
              </CardDescription>
            </CardHeader>
            <CardContent>
              {securityStore.isLoading ? (
                ...
                /* Card View - grouped by module prefix */
                <div className="space-y-6">
